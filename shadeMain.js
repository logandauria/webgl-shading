...
function bindVAO (shape, program) {
    ...
    //create and bind VAO	
    let myVAO = gl.createVertexArray();
...
function setUpPhong(program) {
    ...
    //
    // init values
    let lightPosition = glMatrix.vec3.fromValues(5, -5, -2);
    let lightColor = glMatrix.vec3.fromValues(1, 1, 1);
    let ambientLight = glMatrix.vec3.fromValues(0.6, 0.6, 0.6);
    let baseColor = glMatrix.vec3.fromValues(1, 0, 0);
    let specHighlightColor = glMatrix.vec3.fromValues(1,1,1);
    let ka = 1.0;
    let kd = 1.0;
    let ks = 0.9;
    let ke = 20.0;
    ...
    // closer to camera
    glMatrix.mat4.translate(modelMatrix, modelMatrix, glMatrix.vec3.fromValues(0, 0, -4));
...
function setUpCamera(program) {
    ...
    glMatrix.mat4.perspective(projMatrix, radians(75), 1, 0.5, 300);
